...
document.addEventListener('DOMContentLoaded', () => {
    ...
    predictBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const isIndianStock = indianStocks.includes(data.symbol.replace('.NS', ''));
                ...
                // Update explanation
                explanationElement.innerHTML = generateExplanationHTML(data, currencySymbol);
    ...
    function generateExplanationHTML(data, currencySymbol = '$') {
        fetchStockOfTheDay();
        ...
        let html = `
            ...
            by ${currencySymbol}${Math.abs(data.price_change).toFixed(2)} (${data.price_change_percent.toFixed(2)}%).</p>
            <p>The suggested buy price of ${currencySymbol}${data.buy_price} is calculated as 2% below the predicted price, while the suggested sell price of ${currencySymbol}${data.sell_price} is 2% above the predicted price. These values are influenced by various factors including market trends, historical performance, and technical indicators.</p>
