...
def get_market_data():
    ...
    try:
        ...
        # Define tickers to track (US and Indian markets)
...
def stock_of_the_day():
    ...
    top_gainers = market_data['topGainers'] # type: ignore
...
def predict():
    try:
        ...
        # Handle Indian stocks - append .NS if missing
        indian_stocks = ['RELIANCE', 'TCS', 'HDFCBANK', 'INFY', 'ICICIBANK',
                        'HINDUNILVR', 'BHARTIARTL', 'ITC', 'KOTAKBANK', 'LT']
        if symbol in indian_stocks and not symbol.endswith('.NS'):
            symbol += '.NS'
            app.logger.info(f"Adjusted Indian stock symbol to: {symbol}")
        ...
        response = {
            ...
            'dates': historical_data.index.strftime('%Y-%m-%d').tolist(),  # Include corresponding dates # type: ignore
